...
void TracePC::DumpCoveragesToFile(FuzzingOptions Options){
    char idx[32];
    for (unsigned int i = 0; i < NumModulesWithInline8bitCounters; i++) {
        memset(idx, 0, 32);
        snprintf(idx, 32, "%u", i);
        std::string Path = DirPlusFile(Options.CurrentCoverageDir, idx);
        WriteToFile( Unit(ModuleCounters[i].Start,
                            ModuleCounters[i].Stop), Path);
    }
}
...
/*
    // unless we can sync this, don't use it for exec-hash (won't match between slaves)    

    // ValueProfileMap hash
    uint8_t* byteptr = (uint8_t*)TPC.ValueProfileMap.GetCoverageMap();
    size_t bytelen = TPC.ValueProfileMap.kMapSizeInWords * sizeof(long unsigned int);
    ComputeSHA1(byteptr, bytelen, Hashes[0]);
    ComputeSHA1(pad, kSHA1NumBytes*2, Hashes[1]);    // chain hash
*/
