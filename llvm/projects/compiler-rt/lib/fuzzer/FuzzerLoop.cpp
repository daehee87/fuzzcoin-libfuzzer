...
bool Fuzzer::RunOne(const uint8_t *Data, size_t Size, bool MayDeleteFile,
  ...
  TPC.CollectFeatures([&](size_t Feature) {
    ...
    if (Corpus.AddFeature(Feature, Size, Options.Shrink))
      UniqFeatureSetTmp.push_back(Feature);     // save to current vector if its new feature.
  ...
  // number of features are actually number of non-empty slots
  // feature number is somehow mapped to slot index, 
  // and the slot stores the size of input that triggers the feature
  ...
  // if input triggered same features, then 
  //replace the current one if the new input size is smaller
...
void Fuzzer::ExecuteCallback(const uint8_t *Data, size_t Size) {
  ...
  // fuzzcoin: this includes various maps (e.g., TPC::ValueBitMap::Map).
  TPC.ResetMaps();      
