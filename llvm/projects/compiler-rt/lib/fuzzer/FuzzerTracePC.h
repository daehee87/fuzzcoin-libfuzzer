...
private:
  ...
  // this is inline version of __sancov_trace_pc_8bit_counter
  // we use this as edge coverage info
...
void ForEachNonZeroByte(const uint8_t *Begin, const uint8_t *End,
  ...
  for (; reinterpret_cast<uintptr_t>(P) & StepMask && P < End; P++)
    if (uint8_t V = *P)     // if the counter is not zero
  ...
  for (; P < End; P++)
    if (uint8_t V = *P)
      Handle8bitCounter(FirstFeature, P - Begin, V);    // pass counter value (V) as 3rd arg
}
...
#ifdef FUZZCOIN_DEBUG
    Printf("Counters : %p\n", Counters);
    Printf("GetNumPCs() : %x\n", N);
    Printf("NumInline8bitCounters: %d\n", NumInline8bitCounters);
    Printf("NumModulesWithInline8bitCounters: %d\n", NumModulesWithInline8bitCounters);
    Printf("UseValueProfile: %d\n", UseValueProfile);
#endif
...
#ifdef FUZZCOIN_DEBUG
    Printf("Module Counters : %p-%p\n", ModuleCounters[i].Start, ModuleCounters[i].Stop);
    int len = ModuleCounters[i].Stop - ModuleCounters[i].Start;
    char* ptr = (char*)(ModuleCounters[i].Start);
    for(int i=0; i<len; i++){
        Printf("[%d]", ptr[i]);
    }
    Printf("\n");
